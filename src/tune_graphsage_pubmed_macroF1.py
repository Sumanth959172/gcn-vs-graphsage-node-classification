...
# -----------------------------
# Full-batch GraphSAGE (2-layer) + BN + Dropout
# -----------------------------
class GraphSAGE(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, dropout, aggr="mean"):
        super().__init__()
...
def train_one_run(
    ...
    model = GraphSAGE(in_channels, hidden_dim, out_channels, dropout, aggr=aggr).to(device)
...
def main():
    ...
    # GraphSAGE aggregation type (keep fixed for now)
    aggr = "mean"

    # Search space (PubMed-friendly)
    space = {
    ...
    out_dir = "experiments/tune_pubmed_graphsage_macroF1"
    ...
    # Feature normalization (consistent with your GCN runs)
    x = data.x
    ...
    print(f"Model: GraphSAGE (aggr={aggr}) | Dataset: {dataset_name}")
