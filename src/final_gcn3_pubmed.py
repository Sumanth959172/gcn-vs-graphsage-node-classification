...
def main():
    # 🔧 Update this if your tuning folder name differs
    best_path = "experiments/tune_pubmed_gcn3_macroF1/best.json"
    ...
    # Feature normalization (same as tuning)
    x = data.x
    data.x = (x - x.mean(dim=0)) / (x.std(dim=0) + 1e-6)
    data = data.to(device)

    # Final seeds (5 is a good standard for MSc)
    seeds = [0, 1, 2, 3, 4]
    ...
    out_dir = "experiments/final_pubmed_gcn3"
    os.makedirs(out_dir, exist_ok=True)

    per_seed = []
    print("FINAL EVAL PubMed GCN3")
    print("Using tuned cfg:", {"hidden_dim": hidden_dim, "dropout": dropout, "lr": lr, "weight_decay": weight_decay})
    print("Device:", device)
    print("Seeds:", seeds)
