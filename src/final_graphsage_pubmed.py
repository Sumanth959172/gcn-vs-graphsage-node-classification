...
def main():
    # Tuned config from your best.json
    cfg = {
        "hidden_dim": 256,
        "dropout": 0.7,
        "lr": 0.003,
    ...
    out_dir = "experiments/final_pubmed_graphsage"
    ...
    # Feature normalization (consistent with your other runs)
    x = data.x
    data.x = (x - x.mean(dim=0)) / (x.std(dim=0) + 1e-6)
    data = data.to(device)

    print("FINAL EVAL PubMed GraphSAGE")
    print("Using cfg:", cfg)
    print("Device:", device)
    print("Seeds:", seeds)
